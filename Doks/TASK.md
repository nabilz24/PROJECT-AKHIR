...
| TASK-105 | Pisah landing/login/dashboard | `/` landing only, `/login` login khusus, `/dashboard/*` dashboard | Medium | TASK-104 | - GET / tanpa form login & tanpa panggil auth API; CTA Masuk → /login\n- GET /login memuat form email+password\n- Dashboard EJS per role tidak berubah\n- Full suite hijau + verifikasi live 3 halaman | DONE   |
| TASK-106 | SPA Dashboard UI `/app` | React SPA: login + dashboard semua role + analytics via /api/v1 | High | TASK-104,TASK-105 | - GET /app 200 berisi React root + endpoint dashboard/analytics/me\n- Login inline → /auth/me deteksi role → fetch dashboard sesuai role\n- Mahasiswa/perusahaan/kampus(+dosen) render kartu+tabel; kampus/dosen ada Analytics (bar, heatmap, export CSV)\n- Logout via /auth/logout + hapus token; 401 → kembali login\n- EJS /dashboard/* tidak berubah; full suite hijau | DONE   |
| TASK-107 | Tailwind v4 browser CDN | Ganti Play CDN v3 → @tailwindcss/browser@4 di `/` dan `/app` | Low | TASK-104,TASK-106 | - Kedua halaman memuat tailwindcss/browser, tidak ada sisa cdn.tailwindcss.com\n- Klas utilitas yang dipakai kompatibel v4 (tanpa config kustom)\n- Test CDN diupdate; full suite hijau + verifikasi live | DONE   |
| TASK-108 | Akun testing siap pakai | Seed 4 akun role + profil + project demo via npm run db:seed:test | Medium | TASK-010,TASK-020 | - `npm run db:seed:test` idempotent: 4 akun (mhs/perusahaan/kampus/dosen, password Test123!)\n- Perusahaan terverifikasi; mahasiswa punya 2 skill; 1 project demo aktif + 1 aplikasi pending\n- Login keempat akun berhasil + dashboard mengembalikan data\n- Betulkan path script db:migrate/db:seed di package.json | DONE   |
| TASK-109 | Semua halaman dashboard SPA + audit API | 10 halaman di /app + scripts/audit-endpoints.js + GET /mentor/awaiting | High | TASK-106 | - Audit 74/74 endpoint lolos (happy path + RBAC negatif + halaman), via `npm run audit:api`\n- Halaman baru: Projects, My Skills, Gap, Recommendations, My Projects, Applications, Assess, Notifikasi, Profil (+ Mentor antrian via endpoint baru)\n- JSX tervalidasi Babel asli; full suite hijau; live semua halaman 200 | DONE   |
| TASK-114 | Pages ber-URL + UI/UX overhaul | Hash routing /app#/... + design system + landing dipoles | Medium | TASK-109 | - Tiap halaman punya URL (/app#/projects, #/skills, …) + back/forward browser + document.title\n- UI system: Toast, EmptyState, Skeleton, ConfirmModal (ganti window.confirm), PageHead\n- Responsif: sidebar desktop + topbar mobile; hover/transition konsisten\n- Login /app: quick-fill 4 akun demo; login EJS: hint akun + tautan\n- Landing: sticky nav, hero gradient, fitur 6 kartu, CTA band, footer\n- JSX tervalidasi Babel; 149 test + audit 74/74 hijau; live 3 halaman 200 | DONE   |
